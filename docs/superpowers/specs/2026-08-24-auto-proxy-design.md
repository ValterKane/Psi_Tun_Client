...
- применить изменение = **per-core рестарт** затронутого ядра
  ...
  (рестарты по .exe / process_name -- оставить пользователю для ручного подключения)
Прощупываем только хосты и заносим их как доменные правила в xray.

Главное: -- не дергаем Singbox-TUN без КРАЙНЕЙ необходимости.
...
- Параллельность: до N (где N=Кол-во ядер / 2) одновременных проб.
...
- Достаёт кандидата из очереди → `ProbeAsync` → вердикт:
  - `bad`: `!DirectOk`, **или** (`ProxyOk && DirectMs > random(3,4) × ProxyMs`);
...
- Кулдаун и лимиты: 5 мин между пробами хоста; максимум списка хостов условно ограничен (65535 записей)
  (при превышении вытесняем по `LastCheckedAt`).

**TTL-цикл (auto-heal):**
- У каждого авто-правила TTL=24 часа. Спустя это время, повторные заход на тот же ресурс должен триггерить перепроверку: 
...
- Удаляем из списка авто-правил хосты только после второго посещения. В фоне ничего не обновляем и сами не проверяем. Если пользователь за 24 часа не зашел на ресурс, то он остается в списке и будет вытеснен по `LastCheckedAt` механизмом цикла кандидатов.
...
приложения счётчики обнуляются — это ок: повторные 2/3 пробы за сессию). Без существенной нагрузки на систему. Клиент не должен становиться слишком дорогим по `ОЗУ/ЦПУ`
...
#### Общее требования к UI
-  Сделать его менее clunky, выравнить элементы интерфейса, сделать его менее `колхозным`.
...
|---|---|
| Таймаут пробы | 5 с |
| Порог троттлинга | `DirectMs > random(3,4) × ProxyMs` (и `ProxyOk`) |
| Обучение | 2 подряд bad |
| Откат | 3 подряд good |
| TTL перепроверки | 24 ч |
| Кулдаун хоста | 5 мин |
| Лимит авто-хостов | 65535 |
| Параллельных проб | Количество ядер на машине / 2 |
